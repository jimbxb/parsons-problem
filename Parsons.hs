...
import Data.Maybe (catMaybes, fromMaybe, isNothing, maybe)
...
parsons max ans solns = do
  ...
  where
    go t0 curr (remaining, (soln, ans)) = do
      ...
      next <-
        timeout (fromInteger $ min maxTime remTime) $
...
preprocess ans soln =
  let (ans', ansIndents) = indentations ans
      ...
      possibleIndexes =
        zipWith (\i x -> fixup $ map (, i) x) ansIndents $
...
solve maxEdits soln ans =
  ...
  where
    (valid, invalid) =
      partitionEithers $ map (uncurry ((`maybe` Left) . Right)) $ zip [0 ..] ans
      ...
      | otherwise =
        case zip [0 ..] soln `lookup` states of
          ...
          Nothing ->
            ...
            concatMap (\(st, es) -> map (second (: es)) $ edits soln st) states

edits :: [Int] -> [(Int, Int)] -> [([(Int, Int)], Edit)]
edits soln state = swaps ++ indents ++ cycles
  where
    len = length state
    swaps =
      [ ( map snd start ++
